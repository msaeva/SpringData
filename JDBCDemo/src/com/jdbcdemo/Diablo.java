...
public class Diablo {
    public static void main(String[] args) throws SQLException {
       ...
       PreparedStatement query =
               connection.prepareStatement("SELECT user_name, first_name, last_name, COUNT(ug.id) AS games_count FROM users\n" +
                       "JOIN users_games AS ug ON users.id = ug.user_id  WHERE user_name = ?");
                query.setString(1, username);

        ResultSet result = query.executeQuery();

        if (result.next()) {// valid username
